...
export default function RoulettePage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/casino/pages", {
      ...
      body: JSON.stringify({ slug: "/casino-night/roulette" }),
    });
  }, [router, load]);
  ...
  const betValueOptions: Record<BetType, { value: string; label: string }[]> = {
    ...
    dozen: [
      { value: "1", label: "1st Dozen (1–12)" },
      { value: "2", label: "2nd Dozen (13–24)" },
      { value: "3", label: "3rd Dozen (25–36)" },
    ],
  ...
  // Reset betValue when betType changes
            ...
            {spinResult.newAchievement && (
              <div className="mt-2 rounded-xl bg-amber-500/10 border border-amber-500/30 p-2">
                <p className="text-sm font-bold text-amber-300">
                  {spinResult.newAchievement.emoji} {spinResult.newAchievement.name}
                </p>
          ...
          <div className="rounded-2xl bg-white/5 border border-white/10 px-4 py-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {betTypeOptions.map((o) => (
                  <button
                    key={o.value}
                    onClick={() => handleBetTypeChange(o.value)}
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                />
                <button onClick={allIn} className="rounded-xl border border-rose-500/40 bg-rose-500/10 px-3 py-2 text-rose-400 text-sm hover:bg-rose-500/20 transition-colors">
                  All In
                </button>
              </div>
            ...
            <button
              onClick={spin}
              disabled={spinning || !parseInt(betAmount)}
