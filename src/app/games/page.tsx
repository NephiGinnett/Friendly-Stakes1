  ...
  return (
    <div className="min-h-screen pb-24">
      ...
      <div className="max-w-lg mx-auto px-4 pt-6 space-y-3">
        {(Object.values(GAME_REGISTRY) as (typeof GAME_REGISTRY)[keyof typeof GAME_REGISTRY][]).map((game) => (
          <Link
            key={game.id}
            href={`/games/${game.id}`}
            className="block bg-white/5 border border-white/10 hover:border-violet-500/40 hover:bg-white/8 rounded-2xl p-5 transition-all"
          >
