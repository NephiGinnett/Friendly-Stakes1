...
export async function POST(req: Request) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // Add cost to jackpot regardless
    await tx.houseConfig.update({
      where: { id: 1 },
      data: { scratchJackpot: { increment: cost } },
    });
    ...
    if (isJackpot) {
      // Award jackpot and reset pot
      await tx.user.update({
        where: { id: user.id },
        data: { points: { increment: jackpotPool } },
      });
      ...
      if (!existing) {
        await tx.userAchievement.create({
          data: { userId: user.id, achievementId: "scratch_jackpot" },
        });
