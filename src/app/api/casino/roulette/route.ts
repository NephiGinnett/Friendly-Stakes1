...
export async function GET() {
  ...
  const recent = await prisma.rouletteGame.findMany({
...
export async function POST(req: Request) {
  ...
  const net = winAmount - betAmount; // can be negative
    ...
    if (isAllIn) {
      ...
      if (!existing) {
        await tx.userAchievement.create({
          data: { userId: user.id, achievementId: "casino_all_in" },
        });
        newAchievement = "casino_all_in";

        // Award the 300 pts for this achievement
