...
type TeamBlock = { team: Team; record: TeamRecord; upcoming: Match[]; recent: Match[]; allMatches: Match[] };
type PageData = TeamBlock & { proxy: TeamBlock | null };
...
function TeamSection({ team, record, upcoming, recent, allMatches, isProxy }: TeamBlock & { isProxy: boolean }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card space-y-3">
        <div>
          <p className="text-xs font-mono text-slate-500 uppercase tracking-widest">Bet on {isProxy ? "Your Proxy" : "Your Team"}</p>
...
export default function TeamRecordPage() {
  ...
  const active = tab === "proxy" && data.proxy ? data.proxy : data;
    ...
    <div className="min-h-screen pb-20">
      <header className="sticky top-0 z-40 bg-[rgb(15,15,22)]/90 backdrop-blur-lg border-b border-white/5">
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-white">{activeTeam.flag} {activeTeam.name}</h1>
            <p className="text-xs text-slate-500">
              {activeTeam.confederation} · {isProxy ? "Proxy Team" : "Your Allegiance"} · World Cup 2026
            </p>
          </div>
        </div>
      </header>
